...
JNIEXPORT jstring
JNICALL
Java_com_example_beipush_MainActivity_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
...
Java_com_example_beipush_MainActivity_start_1encode_1push(JNIEnv *env, jobject thiz,
    ...
    if (ret < 0) {
        ...
        LOGE("Couldn't open file %s: %d(%s)", inUrl, ret, buf);
        LOGE("打开文件失败:%d", ret);
        return ret;
    }
    ...
    while (1) {
        ...
        if (pkt.stream_index == videoindex) {
            LOGI("Send %lld video frames to output URL\n", frame_index);
            frame_index++;
        }
        //回调数据
